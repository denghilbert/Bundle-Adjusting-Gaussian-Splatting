...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, specular=None, hybrid=False, distortion_params=None, u_distortion=None, v_distortion=None, u_radial=None, v_radial=None, affine_coeff=None, poly_coeff=None):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        #if idx != 38: continue#cube
        angle = pi / 80

        cam_center_fixed = view.get_camera_center().clone()

        so3_noise = torch.tensor([-angle*10, -angle*0, angle*0])
        ...
        for i in range(20):
            ...
            torchvision.utils.save_image(output.squeeze(0), os.path.join(render_path, f'perspective_frame{i}_{0:05d}'.format(idx) + ".png"))
            torchvision.utils.save_image(rendering, os.path.join(render_path, f'frame{i}_fish_{0:05d}'.format(idx) + ".png"))
