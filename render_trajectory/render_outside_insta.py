...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, specular=None, hybrid=False):
    ...
    for pose in render_poses:
        ...
        so3 = lie.so3_to_SO3(so3_noise).cpu().detach().numpy()
        view_tmp = Camera(view.colmap_id, so3@pose[:3, :3].transpose(), -pose[:3, :3].transpose()@pose[:, 3], view.intrinsic_matrix.cpu().numpy(), view.FoVx, view.FoVy, view.focal_x, view.focal_y, view.original_image.cpu(), None, view.image_name, view.uid, test_outside_rasterizer=True, orig_fov_w=view.original_image.shape[2], orig_fov_h=view.original_image.shape[1])
