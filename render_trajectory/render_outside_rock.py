...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, specular=None, hybrid=False):
    ...
    sample_width = int(width / 32)
    sample_height = int(height/ 32)
    K = views[0].get_K
    ...
    view1 = Camera(view.colmap_id, view.R, view.T, view.intrinsic_matrix.cpu().numpy(), view.FoVx, view.FoVy, view.focal_x, view.focal_y, view.original_image.cpu(), None, view.image_name, view.uid)
    so3_noise = torch.tensor([-angle*look_around_scale, -angle*0, angle*0])
    so3 = lie.so3_to_SO3(so3_noise).cpu().detach().numpy()
    tmp_R = so3 @ view1.R
    view1.reset_extrinsic(tmp_R, tmp_T)
    view2 = Camera(view.colmap_id, view.R, view.T, view.intrinsic_matrix.cpu().numpy(), view.FoVx, view.FoVy, view.focal_x, view.focal_y, view.original_image.cpu(), None, view.image_name, view.uid)
    so3_noise = torch.tensor([angle*look_around_scale, -angle*0, angle*0])
    so3 = lie.so3_to_SO3(so3_noise).cpu().detach().numpy()
    tmp_R = so3 @ view2.R
    view2.reset_extrinsic(tmp_R, tmp_T)
    view3 = Camera(view.colmap_id, view.R, view.T, view.intrinsic_matrix.cpu().numpy(), view.FoVx, view.FoVy, view.focal_x, view.focal_y, view.original_image.cpu(), None, view.image_name, view.uid)
    so3_noise = torch.tensor([angle*0, -angle*look_around_scale, angle*0])
    so3 = lie.so3_to_SO3(so3_noise).cpu().detach().numpy()
    tmp_R = so3 @ view3.R
    view3.reset_extrinsic(tmp_R, tmp_T)
    view4 = Camera(view.colmap_id, view.R, view.T, view.intrinsic_matrix.cpu().numpy(), view.FoVx, view.FoVy, view.focal_x, view.focal_y, view.original_image.cpu(), None, view.image_name, view.uid)
    so3_noise = torch.tensor([angle*0, angle*look_around_scale, angle*0])
    so3 = lie.so3_to_SO3(so3_noise).cpu().detach().numpy()
    tmp_R = so3 @ view4.R
    ...
    render_poses = generate_render_path(view_boundary, focal=5, comps=comps, N=10)
    ...
    #    so3_noise = torch.tensor([angle*0, angle*30, angle*10]) # cube
    #    t_noise = torch.tensor([angle*0, angle*0, angle*0]).numpy()
    #    so3 = lie.so3_to_SO3(so3_noise).cpu().detach().numpy()
    #    view_tmp = Camera(view.colmap_id, so3@pose[:3, :3].transpose(), -pose[:3, :3].transpose()@pose[:, 3], view.intrinsic_matrix.cpu().numpy(), view.FoVx, view.FoVy, view.focal_x, view.focal_y, view.original_image.cpu(), None, view.image_name, view.uid, test_outside_rasterizer=True)
