...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, use_wandb=False, random_init=False, hybrid=False, opt_cam=False, opt_shift=False, no_distortion_mask=False, opt_distortion=False, start_vignetting=10000000000, opt_intrinsic=False, r_t_noise=[0., 0.], r_t_lr=[0.001, 0.001], global_alignment_lr=0.001, extra_loss=False, start_opt_lens=1, extend_scale=2., control_point_sample_scale=8., outside_rasterizer=False, abs_grad=False, densi_num=0.0002, if_circular_mask=False, flow_scale=[1., 1.], render_resolution=1., apply2gt=False, iresnet_lr=1e-7, opacity_threshold=0.005, table1=False):
    ...
    with torch.no_grad():
        ...
        # quaternion -> R -> R.t()
        new_cam = Camera(
            viewpoint_cam.colmap_id, viewpoint_cam.R, viewpoint_cam.T, viewpoint_cam.intrinsic_matrix_numpy, viewpoint_cam.FoVx, viewpoint_cam.FoVy, viewpoint_cam.focal_x, viewpoint_cam.focal_y, viewpoint_cam.original_image_pil, None, viewpoint_cam.fish_gt_image_pil, viewpoint_cam.image_name, viewpoint_cam.uid, depth=None, ori_path=viewpoint_cam.ori_path, outside_rasterizer=viewpoint_cam.outside_rasterizer, test_outside_rasterizer=viewpoint_cam.test_outside_rasterizer, orig_fov_w=viewpoint_cam.orig_fov_w, orig_fov_h=viewpoint_cam.orig_fov_h, original_image_resolution=viewpoint_cam.original_image_resolution, fish_gt_image_resolution=viewpoint_cam.fish_gt_image_resolution, flow_scale=viewpoint_cam.flow_scale, apply2gt=viewpoint_cam.apply2gt, render_resolution=viewpoint_cam.render_resolution, cubemap=cubemap)
        #### TODO

        render_pkg = render(new_cam, gaussians, pipe, background, mlp_color, shift_factors, iteration=iteration, hybrid=hybrid, global_alignment=scene.getGlobalAlignment())
        image, viewspace_point_tensor, visibility_filter, radii = render_pkg["render"], render_pkg["viewspace_points"], render_pkg["visibility_filter"], render_pkg["radii"]
        os.makedirs(os.path.join(scene.model_path, 'trajectory'), exist_ok=True)
        torchvision.utils.save_image(image, os.path.join(scene.model_path, 'trajectory/test.png'))
        torchvision.utils.save_image(new_cam.fish_gt_image, os.path.join(scene.model_path, 'trajectory/test_fish.png'))
