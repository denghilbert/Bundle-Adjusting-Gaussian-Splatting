...
def init_from_colmap(scene, dataset, optimizer_lens_net, lens_net, scheduler_lens_net, control_point_sample_scale, flow_scale):
    ...
    sample_width = int(width / 80)
    sample_height = int(height / 80)
    ...
    for param_group in optimizer_lens_net.param_groups:
        param_group['lr'] = 1e-7# insta
        #param_group['lr'] = 1e-8# insta
        #param_group['lr'] = 5e-9# insta
        #param_group['lr'] = 1e-9# insta
        #param_group['lr'] = 1e-6# fisheyenerf dataset
