...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, specular=None, hybrid=False, distortion_params=None, u_distortion=None, v_distortion=None, u_radial=None, v_radial=None, affine_coeff=None, poly_coeff=None, tmp_cam=None):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        #angle = pi / 80
        #if idx != 79: continue#gene table
        if idx != 38: continue#my table
        angle = pi / 100

        cam_center_fixed = view.get_camera_center().clone()

        so3_noise = torch.tensor([-angle*0, angle*0, angle*0])
        t_noise = torch.tensor([angle*0, -1, -0.5]).numpy()# gene
        t_noise = torch.tensor([0, -1.5, 1]).numpy()# my table
        ...
        look_around_scale = 50#gene_table
        look_around_scale = 10#my_table
        ...
        view1 = Camera(view.colmap_id, view.R, view.T, tmp_cam.intrinsic_matrix.cpu().numpy(), tmp_cam.FoVx, tmp_cam.FoVy, tmp_cam.original_image.cpu(), None, view.image_name, view.uid)
        so3_noise = torch.tensor([-angle*look_around_scale, -angle*0, angle*0])
        so3 = lie.so3_to_SO3(so3_noise).cpu().detach().numpy()
        tmp_R = so3 @ view1.R
        #view1.rotate_at_current_location(tmp_R, cam_center_fixed)
        ...
        view2 = Camera(view.colmap_id, view.R, view.T, tmp_cam.intrinsic_matrix.cpu().numpy(), tmp_cam.FoVx, tmp_cam.FoVy, tmp_cam.original_image.cpu(), None, view.image_name, view.uid)
        so3_noise = torch.tensor([angle*look_around_scale, -angle*0, angle*0])
        so3 = lie.so3_to_SO3(so3_noise).cpu().detach().numpy()
        tmp_R = so3 @ view2.R
        #view2.rotate_at_current_location(tmp_R, cam_center_fixed)
        ...
        view3 = Camera(view.colmap_id, view.R, view.T, tmp_cam.intrinsic_matrix.cpu().numpy(), tmp_cam.FoVx, tmp_cam.FoVy, tmp_cam.original_image.cpu(), None, view.image_name, view.uid)
        so3_noise = torch.tensor([angle*0, -angle*look_around_scale, angle*0])
        so3 = lie.so3_to_SO3(so3_noise).cpu().detach().numpy()
        tmp_R = so3 @ view3.R
        #view3.rotate_at_current_location(tmp_R, cam_center_fixed)
        ...
        view4 = Camera(view.colmap_id, view.R, view.T, tmp_cam.intrinsic_matrix.cpu().numpy(), tmp_cam.FoVx, tmp_cam.FoVy, tmp_cam.original_image.cpu(), None, view.image_name, view.uid)
        so3_noise = torch.tensor([angle*0, angle*look_around_scale, angle*0])
        so3 = lie.so3_to_SO3(so3_noise).cpu().detach().numpy()
        tmp_R = so3 @ view4.R
        #view4.rotate_at_current_location(tmp_R, cam_center_fixed)
        ...
        for pose in render_poses:
            #so3_noise = torch.tensor([angle*0, angle*20, angle*0]) # gene
            #so3_noise = torch.tensor([angle*2, angle*40, angle*0]) # cube
            so3_noise = torch.tensor([angle*30, -angle*30, -angle*10]) # my table
            ...
            view_tmp = Camera(view.colmap_id, so3@pose[:3, :3].transpose(), -pose[:3, :3].transpose()@pose[:, 3], tmp_cam.intrinsic_matrix.cpu().numpy(), tmp_cam.FoVx, tmp_cam.FoVy, tmp_cam.original_image.cpu(), None, view.image_name, view.uid)
            camera_list.append(view_tmp)

        for idx, view in enumerate(camera_list):
        #for idx, view in enumerate([view, view1, view2, view3, view4]):
            #so3_noise = torch.tensor([angle*10, angle*0, angle*0])#gene
            so3_noise = torch.tensor([angle*0, angle*0, angle*0])
            t_noise = torch.tensor([angle*0, angle*0, angle*0]).numpy()
            so3 = lie.so3_to_SO3(so3_noise).cpu().detach().numpy()
            tmp_R = so3 @ view.R
