...
from utils.general_utils import strip_symmetric, build_scaling_rotation, flip_align_view, get_minimum_axis
...
class GaussianModel:
    ...
    def generate_random_points_3d(self, min_corner, max_corner, n):
        # Convert corners to tensors
        min_corner = torch.tensor(min_corner)
        max_corner = torch.tensor(max_corner)

        # Generate random points
        random_points = torch.rand(n, 3) * (max_corner - min_corner) + min_corner

        return random_points

    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float, random_init = False):
        ...
        if random_init:
            max_corner = (np.mean(pcd.points[:, 0]) + np.std(pcd.points[:, 0]), np.mean(pcd.points[:, 1]) + np.std(pcd.points[:, 1]), np.mean(pcd.points[:, 2]) + np.std(pcd.points[:, 2]))
            min_corner = (np.mean(pcd.points[:, 0]) - np.std(pcd.points[:, 0]), np.mean(pcd.points[:, 1]) - np.std(pcd.points[:, 1]), np.mean(pcd.points[:, 2]) - np.std(pcd.points[:, 2]))
            fused_point_cloud = self.generate_random_points_3d(max_corner, min_corner, fused_point_cloud.shape[0]).cuda()
            fused_color = self.generate_random_points_3d((2, 2, 2), (-2, -2, -2), fused_point_cloud.shape[0]).cuda()
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_feature_asg, new_normal, new_normal2):
        d = {"xyz": new_xyz,
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        new_feature_asg = self._features_asg[selected_pts_mask].repeat(N, 1)
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        new_feature_asg = self._features_asg[selected_pts_mask]
    ...
    def add_densification_stats(self, viewspace_point_tensor, viewspace_point_tensor_densify, update_filter, abs_grad):
        if abs_grad:
            self.xyz_gradient_accum[update_filter] += torch.norm(viewspace_point_tensor_densify.grad[update_filter,:2], dim=-1, keepdim=True)
        else:
