...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int, asg_degree: int, opt_shift: bool):
    ...
    def load_ply(self, path):
        ...
        if self.opt_shift:
            import pdb;pdb.set_trace()
            self.shift_outside_factors = nn.Parameter(torch.tensor(xyz, dtype=torch.float, device="cuda").requires_grad_(True))
    ...
    def add_densification_stats(self, viewspace_point_tensor, viewspace_point_tensor_densify, update_filter, abs_grad):
        if abs_grad:
            assert viewspace_point_tensor_densify != None
