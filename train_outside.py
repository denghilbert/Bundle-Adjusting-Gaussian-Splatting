    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        with torch.no_grad():
            ...
            if not outside_rasterizer:
                ...
                P_sensor = None
            training_report(tb_writer, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, render, (pipe, background, mlp_color), lens_net, opt_distortion, P_view_insidelens_direction, P_sensor, outside_rasterizer, flow_scale, flow_apply2_gt_or_img, apply2gt)
...
def training_report(tb_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs, lens_net, opt_distortion, P_view_insidelens_direction, P_sensor, outside_rasterizer, flow_scale, flow_apply2_gt_or_img, apply2gt):
    if tb_writer:
            ...
            for config in validation_configs:
                ...
                if config['cameras'] and len(config['cameras']) > 0:
                    ...
                    for idx, viewpoint in enumerate(config['cameras']):
                        ...
                        torchvision.utils.save_image(image, os.path.join(scene.model_path, 'training_val_{}/renderred/{}'.format(iteration, idx) + "_" + name + ".png"))
                        if outside_rasterizer:
                            if not apply2gt:
                                image = F.grid_sample(
                                    image.unsqueeze(0),
                                ...
                                #    align_corners=True,
                                #).squeeze(0)
                                torchvision.utils.save_image(gt_image, os.path.join(scene.model_path, 'training_val_{}/gt/perspective_{}'.format(iteration, idx) + "_" + name + ".png"))
                                torchvision.utils.save_image(image*mask, os.path.join(scene.model_path, 'training_val_{}/renderred/masked_{}'.format(iteration, idx) + "_" + name + ".png"))
                        torchvision.utils.save_image(viewpoint.fish_gt_image, os.path.join(scene.model_path, 'training_val_{}/gt/{}'.format(iteration, idx) + "_" + name + ".png"))
